...
def encode(listOfValues, listOfCat):
    return " - ".join("({} : {})".format(listOfCat[i], listOfValues[i]) for i in range(len(listOfValues)) )

def stock_list(listOfArt, listOfCat):
    if len(listOfArt) == 0 or len(listOfCat) == 0:
        return ""
    
    result = [0] * len(listOfCat)

    for entry in listOfArt :
        try:
            value_index = listOfCat.index(entry[0])
            splited=entry.split(' ')
            result[value_index] += int(splited[1])    
        except:
            value_index = -1

    return encode(result, listOfCat)
